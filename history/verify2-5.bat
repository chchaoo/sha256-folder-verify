...
REM ------------------------------------------------------------
REM Final update after ALL SOURCE files are processed
REM ------------------------------------------------------------

set /a CURRENT=SOURCE_COUNT
set /a FILE_PERCENT=100
set /a SIZE_PERCENT=100

if !SOURCE_COUNT! GTR 0 (
    set "REL="
    for /f "usebackq delims=" %%R in ("%SOURCE_LIST%") do set "REL=%%R"
    set "FILE=%SOURCE%\!REL!"
...
REM ------------------------------------------------------------
REM Final update after ALL TARGET files are processed
REM ------------------------------------------------------------

set /a CURRENT=TARGET_COUNT
set /a FILE_PERCENT=100
set /a SIZE_PERCENT=100

if !TARGET_COUNT! GTR 0 (
    set "REL="
    for /f "usebackq delims=" %%R in ("%TARGET_LIST%") do set "REL=%%R"
    set "FILE=%TARGET%\!REL!"
...
<nul set /p "=!ESC![2K"
<nul set /p "=File progress: !CURRENT! / !TOTAL!	"
echo [!FILE_BAR!] !FILE_PERCENT!%%

REM Clear and rewrite size progress line
<nul set /p "=!ESC![2K"
<nul set /p "=Size progress:		"
echo [!SIZE_BAR!] !SIZE_PERCENT!%%
