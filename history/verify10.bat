...
#
# It also keeps the timing model used for the time estimate. The
# time one file takes is modelled as
#
#     seconds = A + K * size
#
# A is the fixed cost of a file (finding it on disk, opening it,
# the seek to its first block) and K is seconds per MiB, the
# inverse of the steady read speed of the disk. Every finished
# file is one sample of (size, seconds), and A and K come from a
# least squares fit over the samples. The reader only keeps the
# five running sums the fit needs. Older samples fade out with a
# time constant of two minutes, so the model follows the disk when
# its speed drifts, for example from the fast outer tracks to the
# slower inner ones.
...
#  Time estimate
#
#  For each side:  remaining = A * (files not yet started)
#                            + K * (MiB not yet read)
...
#  until the target has its own figures it borrows the source's,
#  because it is the same disk.
# ------------------------------------------------------------

# A and K from the running sums, or $null while there is nothing
# to go on yet. LiveK is seconds per MiB measured over all that
# this side read so far; it covers the start, when no file has
# been finished yet, for example inside one very large file.
function Get-TimingModel {
    param($Sums, [double]$LiveK)

    if ($Sums -and $Sums[0] -ge 3) {
        $w = [double]$Sums[0]; $sx = [double]$Sums[1]; $sy = [double]$Sums[2]; $sxx = [double]$Sums[3]; $sxy = [double]$Sums[4]
        $den = $w * $sxx - $sx * $sx

        # A real fit needs files of clearly different sizes. When they
        # are all about the same size, the fixed cost and the read
        # time cannot be told apart, and the whole time is put on the
        # size instead.
        if ($sx -gt 0 -and $den -gt (0.05 * $w * $sxx)) {
            $k = ($w * $sxy - $sx * $sy) / $den
            $a = ($sy - $k * $sx) / $w
            if ($k -gt 0 -and $a -ge 0) { return [pscustomobject]@{ A = $a; K = $k } }
        }
        # No usable fit. The overall read speed of this side still
        # gives K, and whatever time is left over per file becomes A.
        # Without that speed there is nothing safe to say yet: putting
        # all the time on the size would turn a few slow small files
        # into hours for the large ones still waiting.
        if ($LiveK -gt 0) {
            $a = ($sy - $LiveK * $sx) / $w
            if ($a -lt 0) { $a = 0.0 }
            return [pscustomobject]@{ A = $a; K = $LiveK }
        }
        return $null
    }

    if ($LiveK -gt 0) { return [pscustomobject]@{ A = 0.0; K = $LiveK } }
    return $null
}
...
function Get-SideSeconds {
    ...
    return ($Model.A * $FilesLeft + $Model.K * ($BytesLeft / 1048576.0))
}
...
function Invoke-HashAll {
    ...
    if ($todoCount -gt 0) {
        ...
        # The speed shown next to each side is measured over the last
        # three seconds, from the byte counts the reader publishes.
        $history = @{
            'S' = New-Object 'System.Collections.Generic.Queue[object]'
            'T' = New-Object 'System.Collections.Generic.Queue[object]'
        }
        ...
        $getView = {
            ...
            $h = $history[$Tag]
            $h.Enqueue(@($Now, $sessBytes))
            while ($h.Count -gt 2 -and ($Now - $h.Peek()[0]) -gt 3.0) { $null = $h.Dequeue() }
            $speed = -1.0
            if ($phase -eq 'run') {
                $old  = $h.Peek()
                $span = $Now - $old[0]
                if ($span -ge 0.5) { $speed = ($sessBytes - [long]$old[1]) / $span }
            }

            $average = 0.0
            if ($isDone -and $sideSecs -gt 0.1) { $average = $sessBytes / $sideSecs }

            # No estimate during the first two seconds of a side: the
            # first few files say little about the disk.
            $liveK = 0.0
            $model = $null
            if ($isStarted -and $sideSecs -ge 2.0) {
                if ($sessBytes -gt 1048576) { $liveK = $sideSecs / ($sessBytes / 1048576.0) }
                $model = Get-TimingModel $state['M' + $Tag] $liveK
            }
