...
title SHA-256 Folder Verification v5

REM ============================================================
REM  SHA-256 FOLDER VERIFICATION - v5
REM
...
REM
REM  New in v5:
REM    - files are hashed by several worker threads at once
REM    - progress is saved to verify5_resume.txt next to this
REM      .bat, so an interrupted run can be continued
REM
...
echo.
echo ============================================================
echo             SHA-256 FOLDER VERIFICATION  -  v5
echo ============================================================
echo.
...
set "SELF=%~f0"
set "RESUMEFILE=%~dp0verify5_resume.txt"
...
#  Compiled in memory by verify5.bat.
...
try { $Host.UI.RawUI.WindowTitle = 'SHA-256 Folder Verification v5' } catch { }
...
$Global:ResumeMagic = 'VERIFY5-RESUME-V1'
$Global:BarWidth    = 40
$Global:BlockTop    = -1
$Global:BlockLines  = 0
$Global:CanDraw     = $true
...
try {
    while ($Queue.TryDequeue([ref]$item)) {
        ...
        $nFiles = $nFiles + 1
        $nBytes = $nBytes + $item.Size
        $State['F' + $Index] = $nFiles
        $State['B' + $Index] = $nBytes
    }
} finally {
...
function Show-HashProgress {
    param(
        $Current, [int]$DoneFiles, [int]$TotalFiles,
    ...
    $size = [long]0
    if ($Current) {
        if ($Current.Side -eq 'S') { $side = 'SOURCE' } else { $side = 'TARGET' }
        $name = $Current.Rel
        $size = $Current.Size
    }
...
function Invoke-HashAll {
    ...
    foreach ($side in $sides) {
        ...
        foreach ($file in $side.Tree.Files) {
            ...
            if ($hit -and $hit.Size -eq $file.Size -and $hit.Mtime -eq $file.Mtime) {
                ...
                if ($side.Tag -eq 'S') { $reusedSrc = $reusedSrc + 1 } else { $reusedDst = $reusedDst + 1 }
            } else {
    ...
    if ($todo.Count -gt 0) {
        ...
        while ($true) {
            ...
            $finished  = $true
            for ($i = 0; $i -lt $workers; $i++) {
                $doneFiles = $doneFiles + [int]$state['F' + $i]
                $sessBytes = $sessBytes + [long]$state['B' + $i]
                if (-not $state['D' + $i]) { $finished = $false }
            }
            $doneBytes = $baseBytes + $sessBytes

            Show-HashProgress -Current $state['Current'] -DoneFiles $doneFiles -TotalFiles $totalFiles -DoneBytes $doneBytes -TotalBytes $totalBytes -Elapsed $watch.Elapsed.TotalSeconds -SessionBytes $sessBytes
        ...
        Show-HashProgress -Current $state['Current'] -DoneFiles $totalFiles -TotalFiles $totalFiles -DoneBytes $totalBytes -TotalBytes $totalBytes -Elapsed $watch.Elapsed.TotalSeconds -SessionBytes ($totalBytes - $baseBytes)
        Close-Block
...
function Invoke-Round {
    ...
    if ([System.IO.File]::Exists($ResumePath)) {
        ...
        if ($null -eq $checkpoint) {
            Write-Host ''
            Write-Host '[WARNING] A resume file was found but it is damaged or' -ForegroundColor Yellow
            Write-Host '          was written by another version. Deleting it.' -ForegroundColor Yellow
...
if ([string]::IsNullOrEmpty($resumePath)) {
    $resumePath = Join-Path ([System.IO.Path]::GetDirectoryName($env:SELF)) 'verify5_resume.txt'
}

Write-Host ''
Write-Host '============================================================'
Write-Host '            SHA-256 FOLDER VERIFICATION  -  v5'
Write-Host '============================================================'
Write-Host ''
