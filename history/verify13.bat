...
REM
REM  Sizes are compared before anything is read. A file that is on
REM  both sides with two different sizes cannot be identical, so it
REM  is reported as such and neither side of it is read.
REM
...
function Invoke-HashAll {
    ...
    # Sizes first. A file that is on both sides but with a different
    # size cannot have the same content, so neither side of it is
    # read at all. Only pairs where both sizes are really known take
    # this shortcut: a size that could not be read at scan time is 0,
    # and must not turn a scan error into a size difference. Names
    # that occur twice on one side, differing only by upper/lower
    # case, are read as usual, so the collision is still reported.
    $byRel = @{}
    $dupes = @{}
    ...
    foreach ($rel in $byRel['S'].Keys) {
        if ($dupes.ContainsKey($rel) -or -not $byRel['T'].ContainsKey($rel)) { continue }
        $s = $byRel['S'][$rel]
        $t = $byRel['T'][$rel]
        if ($s.SizeOk -and $t.SizeOk -and $s.Size -ne $t.Size) { $sizeSkip[$rel] = $true }
    }
    ...
    foreach ($side in $sides) {
        ...
        foreach ($file in $side.Tree.Files) {
            if ($sizeSkip.ContainsKey($file.Rel)) {
                $side.Map[$file.Rel] = [pscustomobject]@{ Hash = $null; Size = $file.Size; SizeOnly = $true }
    ...
    if ($skipFiles -gt 0) {
        Write-Host ('  NOT READ  ' + $sizeSkip.Count + ' files differ in size between the two sides, ' + (Format-Size $skipBytes) + ' skipped') -ForegroundColor Yellow
    }
    Write-Host '------------------------------------------------------------'
...
Write-Host ''
Write-Host 'Size first  : a file whose size differs between the two sides'
Write-Host '              is reported at once, without being read.'
Write-Host ''
