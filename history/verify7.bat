...
REM  drives are read at the same time, folders on the same drive
REM  one after the other.
REM
REM  Progress is saved to (name of this .bat)_resume.txt next to
REM  the .bat, so an interrupted run can be continued.
REM
...
set "SELF=%~f0"
set "RESUMEFILE=%~dpn0_resume.txt"
...
#
#  When the two folders are on different drives, the two readers
#  run at the same time, so both drives stay busy. When both
#  folders are on the same drive letter or the same network
#  share, the target reader only starts after the source reader
#  has finished, so that one disk is never read at two places at
#  once.
#
...
# Drive letter or network share of a path, used to decide whether
# the two readers may run at the same time. Two partitions of one
# physical disk, or a SUBST drive, still look like two drives here.
...
# The first line of each folder section. Finished is checked first,
# because the last file name would otherwise stay on screen after
# that side is already complete.
function Get-SideLabel {
    param($Current, [int]$Done, [int]$Total, [bool]$Waiting, [int]$Width)
    if ($Done -ge $Total) { return '(done)' }
    if ($Waiting)         { return '(waiting, same drive as the source)' }
    if ($Current)         { return ((Format-Fit $Current.Rel $Width) + '   ' + (Format-Size $Current.Size)) }
    return '(starting)'
}
...
function Show-HashProgress {
    ...
    # 11 characters of label, 3 spaces, up to 12 for the size, and the
    # last console column is never written to.
    $nameWidth = 45
    try { $nameWidth = [Console]::BufferWidth - 27 } catch { }
    if ($nameWidth -lt 20) { $nameWidth = 20 }

    $sLabel = Get-SideLabel $SrcCurrent $SrcDone $SrcTotal $false       $nameWidth
    $tLabel = Get-SideLabel $DstCurrent $DstDone $DstTotal $DstWaiting  $nameWidth

    # Three groups: source (3 lines), target (3 lines), summary
    # (1 line). No blank line inside a group, one blank line between
    # two groups.
    $lines = @(
        ('  SOURCE : ' + $sLabel),
...
function Invoke-HashAll {
    param($SrcTree, $DstTree, $Known, $Writer, [bool]$Sequential)
    ...
    $todoCount   = $queues['S'].Count + $queues['T'].Count

    Write-Host ''
    Write-Host '------------------------------------------------------------'
    Write-Host ('  HASHING   ' + $totalFiles + ' files, ' + (Format-Size $totalBytes))
    if ($Sequential) {
        Write-Host '  READING   one folder after the other, they are on the same drive'
    } else {
        Write-Host '  READING   both folders at once, one thread each'
    }
    if ($reusedFiles -gt 0) {
    ...
    if ($todoCount -gt 0) {
        ...
        while ($true) {
            ...
            # Same drive: the target reader starts only once the source
            # reader has read its last file.
            if (-not $readers.ContainsKey('T') -and $state['DS']) { & $startReader 'T' }

            $doneS = $baseFiles['S'] + [int]$state['FS']
            $byteS = $baseBytes['S'] + [long]$state['BS']
            $doneT = $baseFiles['T'] + [int]$state['FT']
            $byteT = $baseBytes['T'] + [long]$state['BT']
            $sessBytes = [long]$state['BS'] + [long]$state['BT']
            $waitT     = (-not $readers.ContainsKey('T'))

            Show-HashProgress -SrcCurrent $state['CS'] -DstCurrent $state['CT'] -SrcDone $doneS -SrcTotal $SrcTree.Files.Count -SrcBytes $byteS -SrcTotalBytes $SrcTree.TotalBytes -DstDone $doneT -DstTotal $DstTree.Files.Count -DstBytes $byteT -DstTotalBytes $DstTree.TotalBytes -DstWaiting $waitT -Elapsed $watch.Elapsed.TotalSeconds -SessionBytes $sessBytes
        ...
        Show-HashProgress -SrcCurrent $null -DstCurrent $null -SrcDone $SrcTree.Files.Count -SrcTotal $SrcTree.Files.Count -SrcBytes $SrcTree.TotalBytes -SrcTotalBytes $SrcTree.TotalBytes -DstDone $DstTree.Files.Count -DstTotal $DstTree.Files.Count -DstBytes $DstTree.TotalBytes -DstTotalBytes $DstTree.TotalBytes -DstWaiting $false -Elapsed $watch.Elapsed.TotalSeconds -SessionBytes ($totalBytes - $reusedBytes)
        Close-Block
...
function Invoke-Round {
    ...
    $sequential = ((Get-VolumeKey $source) -eq (Get-VolumeKey $target))
    ...
    $hashed = $null
    try {
        $hashed = Invoke-HashAll -SrcTree $srcTree -DstTree $dstTree -Known $known -Writer $writer -Sequential $sequential
    } finally {
...
if ([string]::IsNullOrEmpty($resumePath)) {
    $resumePath = Join-Path ([System.IO.Path]::GetDirectoryName($env:SELF)) ([System.IO.Path]::GetFileNameWithoutExtension($env:SELF) + '_resume.txt')
}
...
Write-Host '              the same drive are read one after the other.'
Write-Host ''
