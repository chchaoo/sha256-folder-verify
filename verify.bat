...
REM
REM  Files are hashed block by block, and the progress block shows
REM  how far into the current file each side is, so a very large
REM  file is followed to the megabyte instead of sitting at the
REM  same number until it is finished.
REM
REM  The two trees are matched before anything is read. A file that
REM  is on one side only is missing or extra whatever it contains,
REM  and a file that is on both sides with two different sizes
REM  cannot be identical. Such files are reported without being
REM  read, only files whose content decides the result are read.
REM
    ...
    while ($stack.Count -gt 0) {
        ...
        foreach ($entry in $entries) {
            ...
            if ($isDir) {
                ...
                $files.Add([pscustomobject]@{ Rel = $rel; Dev = $entry; Size = $size; Mtime = $mtime; SizeOk = $sizeOk })
...
#
# After every block it also publishes the file it is on together
# with how far into that file it is, as ONE value: a pair of
# (file, bytes read of it). The main thread reads that pair in one
# go, so the name and the count it shows always belong to the same
# file. Two separate values could be caught half way through a
# switch to the next file, and would show one file's count against
# another file's size.
#
...
function Get-SideLabel {
    ...
    return (('{0,14}' -f $speed) + '   ' + (Format-Fit $View.Current.Rel $Width))
}

# Progress inside the one file being read right now, so that a
# large file does not sit there for minutes with nothing moving.
# The time left for that file is only added when it still fits in
# the window, rather than being cut off half way.
function Get-FileRow {
    param($View, [int]$Cols)
    $row = '   File  : '
    if ($View.Phase -ne 'run' -or $null -eq $View.Current) { return $row }

    $size = [long]$View.Current.Size
    $got  = [long]$View.FileGot
    if ($got -gt $size) { $got = $size }
    if ($size -gt 0) { $pct = ($got * 100.0) / $size } else { $pct = 100.0 }

    $row = $row + (New-Bar $pct) + ('{0,6:N1}%' -f $pct) + '   ' + (Format-Size $got) + ' / ' + (Format-Size $size)

    if ($View.Speed -gt 0 -and $got -lt $size) {
        $tail = '   ' + (Format-Clock (($size - $got) / $View.Speed)) + ' left'
        if (($row.Length + $tail.Length) -le ($Cols - 1)) { $row = $row + $tail }
    }
    return $row
}
...
function Show-HashProgress {
    ...
    # 11 columns of label, 14 for the speed, 3 spaces, the name, and
    # the last column stays free. The size of the file is shown on
    # the File row below it.
    $nameWidth = $cols - 29
...
function Invoke-HashAll {
    ...
    # Nothing is read whose content cannot change the result.
    #
    # A file that is on one side only is missing in the target or
    # extra in it, whatever it contains, so it is not read.
    #
    # A file that is on both sides but with a different size cannot
    # have the same content, so neither side of it is read either.
    # Only pairs where both sizes are really known take this
    # shortcut: a size that could not be read at scan time is 0, and
    # must not turn a scan error into a size difference.
    #
    # Names that occur twice on one side, differing only by
    # upper/lower case, are read as usual, so the collision is still
    # reported.
    $byRel = @{}
    $dupes = @{}
    foreach ($side in $sides) {
        ...
        foreach ($file in $side.Tree.Files) {
            if ($seen.ContainsKey($file.Rel)) { $dupes[$file.Rel] = $true } else { $seen[$file.Rel] = $file }
        }
        $byRel[$side.Tag] = $seen
    }
    # Relative path -> why it is not read: 'size', 'missing' or 'extra'.
    $skip = @{}
    foreach ($rel in $byRel['S'].Keys) {
        if ($dupes.ContainsKey($rel)) { continue }
        if (-not $byRel['T'].ContainsKey($rel)) { $skip[$rel] = 'missing'; continue }
        $s = $byRel['S'][$rel]
        $t = $byRel['T'][$rel]
        if ($s.SizeOk -and $t.SizeOk -and $s.Size -ne $t.Size) { $skip[$rel] = 'size' }
    }
    foreach ($rel in $byRel['T'].Keys) {
        if ($dupes.ContainsKey($rel)) { continue }
        if (-not $byRel['S'].ContainsKey($rel)) { $skip[$rel] = 'extra' }
    }
    $skipCount = @{ 'size' = 0; 'missing' = 0; 'extra' = 0 }
    foreach ($why in $skip.Values) { $skipCount[$why] = $skipCount[$why] + 1 }

    # Split the rest into "already known from the resume file" and
    ...
    foreach ($side in $sides) {
        ...
        foreach ($file in $side.Tree.Files) {
            if ($skip.ContainsKey($file.Rel)) {
                # No hash. A file on one side only is reported as missing
                # or extra before its hash is ever looked at, and SizeOnly
                # tells the comparison that a missing hash here means
                # "not read on purpose", not "could not be read".
                $side.Map[$file.Rel] = [pscustomobject]@{ Hash = $null; Size = $file.Size; SizeOnly = ($skip[$file.Rel] -eq 'size') }
                $skipFiles = $skipFiles + 1
                $skipBytes = $skipBytes + [long]$file.Size
                continue
            }
    ...
    # What the progress bars count: files whose size differs are not
    # part of the work, so the bars and the estimate leave them out.
    $sideFiles = @{}
    $sideBytes = @{}
    foreach ($tag in @('S', 'T')) {
        $sideFiles[$tag] = $baseFiles[$tag] + $queueFiles[$tag]
        $sideBytes[$tag] = $baseBytes[$tag] + $queueBytes[$tag]
    }
    $totalFiles = $sideFiles['S'] + $sideFiles['T']
    $totalBytes = $sideBytes['S'] + $sideBytes['T']
    ...
    if ($skipFiles -gt 0) {
        $why = @()
        if ($skipCount['size']    -gt 0) { $why += ('{0} differ in size'    -f $skipCount['size']) }
        if ($skipCount['missing'] -gt 0) { $why += ('{0} missing in target' -f $skipCount['missing']) }
        if ($skipCount['extra']   -gt 0) { $why += ('{0} extra in target'   -f $skipCount['extra']) }
        Write-Host ('  NOT READ  ' + $skipFiles + ' files, ' + (Format-Size $skipBytes) + ', their content cannot change the result:') -ForegroundColor Yellow
        Write-Host ('            ' + ($why -join ', ')) -ForegroundColor Yellow
    }
    Write-Host '------------------------------------------------------------'
    ...
    if ($todoCount -gt 0) {
        ...
        $getView = {
            ...
            # The file being read and how far into it the reader is,
            # taken from one pair so that both belong to the same file.
            $pair      = $state['C' + $Tag]
            $current   = $null
            $fileGot   = [long]0
            if ($pair) { $current = $pair[0]; $fileGot = [long]$pair[1] }
...
function Invoke-Round {
    ...
    foreach ($rel in ($srcMap.Keys | Sort-Object)) {
        ...
        $d = $dstMap[$rel]
        # Checked before the missing hash below: these two files were
        # left unread on purpose, they are not unreadable.
        if ($s.SizeOnly -or $d.SizeOnly) {
            $details.Add('[SIZE DIFFERS]       ' + $rel)
            $details.Add('                     source ' + $s.Size + ' bytes, target ' + $d.Size + ' bytes, not read')
            $sizeDiff = $sizeDiff + 1
            continue
        }
    ...
    $passed = ($different -eq 0 -and $sizeDiff -eq 0 -and $missing -eq 0 -and $extra -eq 0 -and $hashErrors -eq 0 -and $dirMissing -eq 0 -and $dirExtra -eq 0 -and $scanErrors -eq 0)
    ...
    Write-Host ('  Different size            : {0}' -f $sizeDiff)
...
Write-Host ''
Write-Host 'Not read    : files that are on one side only, and files whose'
Write-Host '              size differs between the two sides. They are'
Write-Host '              reported at once, their content cannot change'
Write-Host '              the result.'
Write-Host ''
